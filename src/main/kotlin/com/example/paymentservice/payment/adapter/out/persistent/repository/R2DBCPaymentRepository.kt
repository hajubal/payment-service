...
class R2DBCPaymentRepository(
    ...
    private fun insertPaymentEvent(paymentEvent: PaymentEvent): Mono<Long> {
        return databaseClient.sql(INSERT_PAYMENT_EVENT_QUERY)
            ...
            .bind("isPaymentDone", paymentEvent.isPaymentDone())
            .fetch()
            .rowsUpdated()
    }
    ...
    private fun insertPaymentOrders(
        ...
        val valueClauses = paymentEvent.paymentOrders.joinToString(", ") { paymentOrder ->
            "($paymentEventId, ${paymentOrder.sellerId}, '${paymentOrder.orderId}'" +
                    ", ${paymentOrder.productId}, ${paymentOrder.amount}, ${paymentOrder.paymentStatus})"
        }
    ...
    companion object {
        val INSERT_PAYMENT_EVENT_QUERY = """
            INSERT INTO payment_events (buyer_id, order_name, order_id, is_payment_done)
            VALUES (:buyerId, :orderName, :orderId, :isPaymentDone)
        """.trimIndent()
